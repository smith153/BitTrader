...
sub get_indicator : Test(3)
{
  ...
  $self->get_indicator();
  is ref($self->indicator()), "Indicator::StochOsc", "Indicator should be StochOsc";

  system("echo 'Ma' > $file");
  $self->get_indicator();
  is ref($self->indicator()), "Indicator::Ma", "Indicator should be Ma";

  system("echo 'StochFast' > $file");
  $self->get_indicator();
  is ref($self->indicator()), "Indicator::StochFast", "Indicator should be StochFast";
