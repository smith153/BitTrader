package TestsFor::BitTrader::Indicator::StochOsc;
use Test::Most;
use base 'TestsFor';
...
sub startup : Test(startup)
{
  ...
  $test->SUPER::startup;
  my $class = ref $test;
  $class->mk_classdata('default_StochOsc');
...
sub setup : Test(setup)
{
  ...
  $test->default_StochOsc( $test->class_to_test->new(
		k => 2,
		d => 1,
		stoch_size => 3,
        )
...
{ 
  ...
  can_ok $class, 'new';
  isa_ok $test->default_StochOsc, $class;
}
